...
class PgPreparedStatement extends PgStatement implements PreparedStatement {
  ...
  public void setNull(int parameterIndex, int sqlType) throws SQLException {
    ...
    if (parameterIndex < 1 || parameterIndex > preparedParameters.getParameterCount()) {
      throw new SQLDataException(
        ...
        PgSqlState.INVALID_INDICATOR_PARAMETER_VALUE);
    }
    ...
    switch (sqlType) {
      ...
      default:
        // Bad Types value.
        throw new SQLSyntaxErrorException(GT.tr("Unknown Types value."), PgSqlState.WRONG_OBJECT_TYPE);
    }
  ...
  private void setCharacterStreamPost71(@Positive int parameterIndex,
    ...
    if (length < 0) {
      throw new SQLDataException(GT.tr("Invalid stream length {0}.", length),
    ...
    try {
      ...
      throw new SQLException(GT.tr("The JVM claims not to support the {0} encoding.", encoding),
          PgSqlState.SYSTEM_ERROR, uee);
    } catch (IOException ioe) {
      throw new SQLException(GT.tr("Provided InputStream failed."), PgSqlState.SYSTEM_ERROR,
          ioe);
    }
  }
  ...
  private void setPGobject(@Positive int parameterIndex, PGobject x) throws SQLException {
    ...
    if (oid == Oid.UNSPECIFIED) {
      throw new SQLSyntaxErrorException(GT.tr("Unknown type {0}.", typename),
  ...
  private void setMap(@Positive int parameterIndex, Map<?, ?> x) throws SQLException {
    ...
    if (oid == Oid.UNSPECIFIED) {
      throw new SQLSyntaxErrorException(GT.tr("No hstore extension installed."),
          PgSqlState.DATATYPE_MISMATCH);
    }
  ...
  public void setObject(@Positive int parameterIndex, @Nullable Object in,
    ...
    switch (targetSqlType) {
      ...
      case Types.OTHER:
        ...
        break;
      default:
        throw new SQLSyntaxErrorException(GT.tr("Unsupported Types value: {0}", targetSqlType),
            PgSqlState.DATATYPE_MISMATCH);
    }
  }
  ...
  private static SQLSyntaxErrorException cannotCastException(final String fromType, final String toType) {
    return cannotCastException(fromType, toType, null);
  }

  private static SQLSyntaxErrorException cannotCastException(final String fromType,
      final String toType, final @Nullable Exception cause) {
    String message = GT.tr("Cannot convert an instance of {0} to type {1}", fromType, toType);
    if (cause != null) {
      return new SQLSyntaxErrorException(message, PgSqlState.CANNOT_COERCE, cause);
    }
    return new SQLSyntaxErrorException(message, PgSqlState.CANNOT_COERCE);
  }
  ...
  public void setObject(@Positive int parameterIndex, @Nullable Object x) throws SQLException {
    ...
    if (x == null) {
      ...
      throw new SQLSyntaxErrorException(GT.tr(
          ...
          x.getClass().getName()), PgSqlState.DATATYPE_MISMATCH);
    }
  }
  ...
  protected long createBlob(int i, InputStream inputStream,
    ...
    try {
      ...
      throw new SQLException(GT.tr("Unexpected error writing large object to database."),
          PgSqlState.SYSTEM_ERROR, se);
    } finally {
      try {
        outputStream.close();
      } catch (Exception e) {
  ...
  private String readerToString(Reader value, int maxLength) throws SQLException {
    try {
      ...
      throw new SQLException(GT.tr("Provided Reader failed."), PgSqlState.SYSTEM_ERROR, ioe);
    }
  }

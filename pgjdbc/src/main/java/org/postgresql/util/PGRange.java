...
public class PGRange<T> extends PGobject implements Serializable, Cloneable {
  ...
  /**
   * Parses a PostgreSQL range string into a PGRange.
   *
   * @param value the PostgreSQL range string (e.g., "[1,10)" or "empty")
   * @param parser function to parse individual bound values
   * @param <T> the type of the range bounds
   * @return the parsed range
   * @throws SQLException if parsing fails
   */
  public static <T> PGRange<T> parse(String value, BoundParser<T> parser) throws SQLException {
    if (value == null || value.isEmpty() || "empty".equalsIgnoreCase(value)) {
      return empty();
    }

    int len = value.length();
    if (len < 3) {
      throw new PSQLException(GT.tr("Invalid range format: {0}", value), PSQLState.DATA_TYPE_MISMATCH);
    }
    ...
    if (firstChar == '[') {
      ...
      throw new PSQLException(GT.tr("Invalid range format, expected '[' or '(': {0}", value),
          PSQLState.DATA_TYPE_MISMATCH);
    }
    ...
    // Find the comma separator, handling quoted values
    String inner = value.substring(1, len - 1);
    int commaPos = findComma(inner);
    if (commaPos < 0) {
      throw new PSQLException(GT.tr("Invalid range format, missing comma: {0}", value),
          PSQLState.DATA_TYPE_MISMATCH);
    }

    String lowerStr = inner.substring(0, commaPos);
    String upperStr = inner.substring(commaPos + 1);

    T lower = lowerStr.isEmpty() ? null : parser.parse(unquote(lowerStr));
    T upper = upperStr.isEmpty() ? null : parser.parse(unquote(upperStr));

    return new PGRange<>(lower, upper, lowerInclusive, upperInclusive);
  }

  /**
   * Finds the position of the comma separator, handling quoted strings.
   */
  ...
  /**
   * Unquotes a value if it's quoted.
   */

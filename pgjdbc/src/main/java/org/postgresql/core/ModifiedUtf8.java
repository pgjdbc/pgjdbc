...
public class ModifiedUtf8 {
  ...
  /**
   * Returns the number of bytes the modified UTF-8 representation of 's' would take. Note that this is just the space for the bytes
   * representing the characters, not the length which precedes those bytes, because different callers represent the length
   * differently, as two, four, or even eight bytes. If {@code shortLength} is true, we'll throw an exception if the string is too
   * long for its length to be represented by a short.
   */
  ...
  /**
   * Encodes the <i>modified UTF-8</i> bytes corresponding to string {@code s} into the byte array {@code dst}, starting at the
   * given {@code offset}.
   */
   ...
   * Throws UTFDataFormatException if {@code s} is too long for a two-byte length.
   */

...
public class Parser {
   ...
   * @throws SQLSyntaxErrorException if given SQL is wrong
   */
  public static String replaceProcessing(String sql, boolean replaceProcessingEnabled,
      boolean standardConformingStrings) throws SQLSyntaxErrorException {
  ...
  private static int escapeFunctionArguments(StringBuilder newsql, String functionName, char[] sql, int i,
    ...
    try {
      ...
      if (targetException instanceof SQLSyntaxErrorException) {
        ...
        throw new SQLSyntaxErrorException(targetException.getMessage(), PgSqlState.SYSTEM_ERROR,
            targetException);
      }
    } catch (IllegalAccessException e) {
      throw new SQLSyntaxErrorException(e.getMessage(), PgSqlState.SYSTEM_ERROR);
    }
    return i;
  }

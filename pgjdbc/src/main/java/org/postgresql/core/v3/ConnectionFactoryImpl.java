...
public class ConnectionFactoryImpl extends ConnectionFactory {
  ...
  static void validateOAuthIssuer(Properties info) throws PSQLException {
    ...
    boolean isHttps;
    try {
      isHttps = "https".equalsIgnoreCase(new URI(issuer).getScheme());
  ...
  private static char @Nullable [] resolveOAuthToken(Properties info) throws PSQLException {
    ...
    if (providerClassName != null && !providerClassName.isEmpty()) {
      ...
      OAuthTokenRequest request = new OAuthTokenRequest(
          PGProperty.OAUTH_USERNAME.getOrDefault(info),
      ...
      char [] token = provider.getToken(request);
          ...
          case PgMessageType.AUTHENTICATION_RESPONSE:
            ...
            switch (areq) {
              ...
              case AUTH_REQ_SASL: {
                ...
                if (oauthOffered && AuthMethod.isAllowed(authMethods, AuthMethod.OAUTH)) {
                  // OAuth
                  ...
                  boolean allowUnencrypted =
                      PGProperty.OAUTH_ALLOW_UNENCRYPTED_CONNECTION.getBoolean(info);
                  oauthAuthenticator = new OAuthAuthenticator(pgStream, allowUnencrypted);
                  /*
                   * For now oauthToken is always non-null. But in the future there may be discovery support,
                   * where empty token is passed to get issuer information. Following code already has support
                   * for that, even though it's not used yet.
                   */
                  ...
                  // No mechanism was selected. If the driver could otherwise handle an offered
                  // mechanism but requireAuth forbids it, surface that specific reason instead of
                  // the generic "not configured or supported" message.
                  if (oauthOffered) {
                  ...
                  throw new PSQLException(
                      GT.tr("The server requested SASL authentication with mechanisms {0}, "
                          + "but none of them configured or supported by the driver.",
                          saslMechanisms),
                      PSQLState.CONNECTION_REJECTED);
                }
                break;

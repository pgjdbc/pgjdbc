...
public class ConnectionFactoryImpl extends ConnectionFactory {
   ...
   * decision, so neither parameter can be sent twice or dropped, and both methods are pure. Keep
   * the decision in one place. While it was split between two callers that read different
   * versions, 42.7.5 sent neither parameter on a connection with an assumed 9.0 or later
   * (PR #3476, fixed by PR #3509).</p>
   */
  static final class InitialSessionParameters {
    ...
    /**
     * Returns whether {@code extra_float_digits} goes in the startup packet. It does when
     * {@code assumeMinServerVersion} is at least 9.0 and below 12, because an 8.x server accepts
     * at most 2 and a v12 server already prints floats in the shortest form that reads back
     * exactly under its default of 1. The bound reads the asserted version rather than the
     * server's, so asserting a version from 9.0 to 11.x against a pre-9.0 server builds a packet
     * that server refuses.
     *
     * <p>Placing the value here spares a post-authentication {@code SET}, which a session that
     * rejects arbitrary SQL, such as Greenplum retrieve mode, cannot run (discussion #4306).</p>
     */

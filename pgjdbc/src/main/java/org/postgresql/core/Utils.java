...
public class Utils {
  ...
  private static void doAppendEscapedLiteral(StringBuilder sbuf, String value,
    ...
    if (standardConformingStrings) {
      ...
      for (int i = 0; i < value.length(); ++i) {
        ...
        if (ch == '\0') {
          throw new SQLDataException(GT.tr("invalid byte sequence: {0}", ch),
   ...
   *
   * @param sbuf StringBuilder as we do not expect any IOException to be thrown.
   * @param value value to append
   */
   ...
   */
  @Deprecated
  public static int parseServerVersionStr(@Nullable String serverVersion)

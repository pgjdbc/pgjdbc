...
public class DriverTest extends TestCase {
  ...
  public void testAcceptsURL() throws Exception {
    ...
    verifyUrl(drv, TestUtil.getPrefix() + "test", "localhost", "5432", "test");
    verifyUrl(drv, TestUtil.getPrefix() + "//localhost/test", "localhost", "5432", "test");
    verifyUrl(drv, TestUtil.getPrefix() + "//localhost:5432/test", "localhost", "5432", "test");
    verifyUrl(drv, TestUtil.getPrefix() + "//127.0.0.1/anydbname", "127.0.0.1", "5432", "anydbname");
    verifyUrl(drv, TestUtil.getPrefix() + "//127.0.0.1:5433/hidden", "127.0.0.1", "5433", "hidden");
    verifyUrl(drv, TestUtil.getPrefix() + "//[::1]:5740/db", "[::1]", "5740", "db");

    // Badly formatted url's
    assertTrue(!drv.acceptsURL("jdbc:postgres:test"));
    assertTrue(!drv.acceptsURL("postgresql:test"));
    assertTrue(!drv.acceptsURL("db"));
    assertTrue(!drv.acceptsURL(TestUtil.getPrefix() + "//localhost:5432a/test"));

    // failover urls
    verifyUrl(drv, TestUtil.getPrefix() + "//localhost,127.0.0.1:5432/test", "localhost,127.0.0.1",
        "5432,5432", "test");
    verifyUrl(drv, TestUtil.getPrefix() + "//localhost:5433,127.0.0.1:5432/test", "localhost,127.0.0.1",
        "5433,5432", "test");
    verifyUrl(drv, TestUtil.getPrefix() + "//[::1],[::1]:5432/db", "[::1],[::1]", "5432,5432", "db");
    verifyUrl(drv, TestUtil.getPrefix() + "//[::1]:5740,127.0.0.1:5432/db", "[::1],127.0.0.1", "5740,5432",
        "db");
  }
  ...
  public void testConnectFailover() throws Exception {
    String url = TestUtil.getPrefix() + "//invalidhost.not.here," + TestUtil.getServer() + ":"

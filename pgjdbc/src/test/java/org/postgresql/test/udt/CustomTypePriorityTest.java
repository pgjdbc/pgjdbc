...
public class CustomTypePriorityTest extends BaseConnectionTest {
  ...
  public void testSetObjectWithoutSqlType() throws Exception {
    ...
    if (baseConnection.getQueryExecutor().getPreferQueryMode() == PreferQueryMode.SIMPLE) {
      ...
      Assert.assertEquals("column type: " + result.getMetaData().getColumnTypeName(1), BigDecimalSQLData.CONSTANT_VALUE, result.getObject("sqldata", Collections.<String, Class<?>>singletonMap("text", BigDecimalSQLData.class)));
    } else {

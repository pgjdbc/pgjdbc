#!/usr/bin/env bash
#
# Trigger a Fedora Copr build for this commit or pull request and wait for it to finish.
# On failure, print the Copr build log so the error appears in the CI step output.
#
# GitHub Actions runs this script. The script Copr itself runs is `build-script` in this
# directory. This one submits the build over the Copr webhook and polls the REST API, the
# same work copr-ci-tooling's copr-gh-actions-submit does, so the workflow no longer
# downloads a remote script at run time.
#
# Required environment:
#   COPR_PR_WEBHOOK    custom webhook URL for pull request builds (public)
...
if [ -n "$pr_id" ]; then
  # actions/checkout leaves the merge ref checked out, so fetch the pull request head
  # and send that commit to Copr instead.
  ref=refs/pull/$pr_id
...
# Copr answers with the build ID alone. Accept nothing else: an error page served with
# status 200 contains numbers too, and polling one of them would follow somebody else's build.
build_id=$(curl -fsS -X POST -H 'Content-Type: application/json' --data "$payload" "$webhook")
case $build_id in
  '' | *[!0-9]*)
    echo "::error::Copr build submission did not return a build ID"
    printf 'response: %s\n' "$build_id"
    exit 1
    ;;
esac
...
# Poll until the build reaches a terminal state, ignoring transient API errors. The loop
# counts its tries rather than running unbounded, so a stuck build cannot hang the job.
api="https://copr.fedorainfracloud.org/api_3/build/$build_id"
state=""
for ((i = 0; i < 240; i++)); do  # 240 tries at 30 seconds, so about 2 hours
  sleep 30
...
# The loop either broke on a failed or canceled build, or ran out of tries.
case $state in
...
# Error markers that dnf, rpmbuild, Maven, and Gradle print.
...
# Print the first error lines with their context, since the first failure is usually the
# root cause. grep --color highlights the matched marker, and GitHub renders the ANSI codes.
...
# An empty source version means the SRPM stage itself failed, so show its log. Once the
# SRPM builds, the failures are in the per-chroot RPM builds below.
if [ -z "$srpm_version" ]; then
  show "SRPM build log" "$repo_url/srpm-builds/$padded/builder-live.log.gz"
fi

# Print the build log of every chroot that got far enough to produce one, each in a
# collapsed group, since several chroots can fail at once.

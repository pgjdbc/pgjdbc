 ...
 */
class PhysicalFullXAConnection {
    ...
    private List<PGFullXAConnection> logicalConnections = new ArrayList<PGFullXAConnection>(2);
     ...
     */
    PhysicalFullXAConnection(final BaseConnection physicalConn, final String user) throws SQLException {
        ...
        if (logger.logDebug()) {
            logger.debug(GT.tr("[{0}] - {1} instantiated", new Object[]{backendPid, PhysicalFullXAConnection.class.getName()}));
        }
    }
     ...
     */
    synchronized Xid getAssociatedXid() {
    ...
    /**
     * Attempts to re-associate this physical connection for the given 
     * logicalConnection and xid, if this physical connection was servicing this
     * logical and this xid.
     * 
     ...
     */
    synchronized boolean reassociate(final PGFullXAConnection logicalConnection, final Xid xid) throws IllegalStateException {
        if (!reserved) {
            if (xid != null) {
     ...
     */
    synchronized boolean associate(final PGFullXAConnection logicalConnection, final Xid xid) throws IllegalStateException {
        if (!reserved) {
            try {
                // Sanity checks.
     ...
     */
    synchronized boolean isCloseable(final PGFullXAConnection logicalConnection) {
            ...
            try {
                ...
                if (connection.isClosed()) {
                    ...
                    closeable = (logicalConnection.getUser().equals(user) && // Same user AND
                                 ...
                                 || // OR 
                                 (associatedXid != null && !isSuspended() && isOnlyLogicalAssociation(logicalConnection)) // Xid in progress, not suspended, and only one logical association.
                            );
                }
            } catch (SQLException sqle) {
                closeable = true;
            }
        } else { // Reserved or unlockable.
            closeable = false;
        }
        return closeable;
    }
     ...
     */
    synchronized boolean isOnlyLogicalAssociation(final PGFullXAConnection logicalConnection) {
     ...
     * @throws IllegalStateException 
     */
    private synchronized void disassociate(final PGFullXAConnection logicalConnection, final Xid xid, final boolean restoreAutoCommit) throws IllegalStateException {
        try {
     ...
     * @throws IllegalStateException 
     */
    synchronized void disassociate(final PGFullXAConnection logicalConnection, final Xid xid) {
     ...
     * @throws IllegalStateException 
     */
    synchronized void disassociate(final PGFullXAConnection logicalConnection) throws IllegalStateException {
     ...
     * @throws IllegalStateException 
     */
    synchronized void disassociate(final Xid xid) {
     ...
     */
    synchronized private void disassociate(boolean restoreAutoCommit) {
        // Clean up the logical connections.
        PGFullXAConnection logical = null;
     ...
     * Disassociates all logical connections and any associated xid from this connection.
     */
    synchronized void disassociate() {
        disassociate(true);
    }
    ...
    public boolean equals(final Object obj) {
        if (obj == null || getClass() != obj.getClass() || this.backendPid != ((PhysicalFullXAConnection)obj).backendPid) {
            return false;
        }
        return true;
    }

...
/**
 * Edge-case {@code macaddr} (6-byte MAC) values: all-zero, all-ones, and a typical address.
 *
 * <p>Read-only ({@link EdgeCase#value()} is {@code null}).
 */
public final class MacaddrEdgeCases {
  /** Every case, in a stable order. */
  public static final List<EdgeCase> ALL = Collections.unmodifiableList(all());

  private MacaddrEdgeCases() {
  }

  private static List<EdgeCase> all() {
    List<EdgeCase> out = new ArrayList<>();
    out.add(at("zero", "00:00:00:00:00:00"));
    out.add(at("all_ones", "ff:ff:ff:ff:ff:ff"));
    out.add(at("typical", "08:00:2b:01:02:03"));
    return out;
  }

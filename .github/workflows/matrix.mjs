...
matrix.addAxis({
  ...
  title: x => x == '' ? '' : 'default_query_timeout ' + x,
  values: [
    '',
    '15'
  ]
...
matrix.setNamePattern([
    ...
    'adaptive_fetch', 'rewrite_batch_inserts', 'default_query_timeout'
]);
...
include.forEach(v => {
  ...
  v.default_query_timeout = v.default_query_timeout.value;
  ...
  if (v.default_query_timeout) {
      testJvmArgs.push(`-DdefaultQueryTimeout=${v.default_query_timeout}`);
  }

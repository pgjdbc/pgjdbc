...
* **sslkey** = String
	...
	*Note:* The key file **must** be in [PKCS-8](https://en.wikipedia.org/wiki/PKCS_8) [DER format](https://wiki.openssl.org/index.php/DER). A PEM key can be converted to DER format using the openssl command:
	...
	`openssl pkcs8 -topk8 -inform PEM -in my.key -outform DER -out postgresql.pk8 -v1 PBE-MD5-DES`
...
* **sslpassword** = String
	...
	If provided will be used by ConsoleCallbackHandler *if you do not provide this you need to add -nocrypt to the openssl command above *

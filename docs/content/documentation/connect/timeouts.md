...
These properties only cover the one-byte "do you speak SSL/GSS encryption?" response. The driver temporarily sets `SO_TIMEOUT` to the response-timeout value for that read, unless an even smaller socket timeout is already active. They do **not** bound the rest of the TLS/GSS handshake. For TLS specifically, once the server replies `S`, pgJDBC hands control to JSSE and temporarily sets the SSL socket's read timeout from `connectTimeout` during `startHandshake()`, then restores `socketTimeout` afterward.
...
When HikariCP is configured with `dataSourceClassName=org.postgresql.ds.PGSimpleDataSource` (the [HikariCP-recommended](https://github.com/brettwooldridge/HikariCP#popular-datasource-class-names) form), it calls `DataSource.setLoginTimeout(seconds)` while initializing the underlying `DataSource`, deriving the value from `HikariConfig.connectionTimeout` as roughly `max(1, (connectionTimeout + 500) / 1000)`. Since `HikariConfig.connectionTimeout` defaults to **30 000 ms**, pgJDBC sees a `loginTimeout` of about `30 s` unless you set `HikariConfig.connectionTimeout` explicitly.
...
`HikariConfig.connectionTimeout` is the client-side wait for `pool.getConnection()` — it bounds how long an application thread blocks waiting to **borrow** a pooled connection. It does **not** directly wrap a single physical JDBC connect attempt. HikariCP's fail-fast check (`initializationFailTimeout >= 0`, the default) and background pool-fill path create physical connections outside that borrow-timeout path, so those attempts are bounded by the driver-side timeouts (`connectTimeout`, `loginTimeout`) plus any translation HikariCP has already performed via `setLoginTimeout`. If those leave the driver waiting indefinitely on a half-open multi-host URL, application startup or refill can still hang irrespective of the pool's borrow timeout.
...
`hostRecheckSeconds` is not itself a timeout; it is the TTL on cached host state. The right value depends on how aggressively you want the driver to re-probe failed hosts relative to your retry cadence and fail-over expectations. The fail-over story it participates in is on [Connection fail-over](/documentation/connect/failover/).

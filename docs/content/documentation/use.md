...
`pgjdbc.protocolHardeningMode` (`fail` | `disable`, default: `fail`). Whether the driver enforces three limits it chose for itself: `maxServerTextMessageSize`, the built-in `CopyData` limit that applies while `maxCopyDataSize` is unset, and the 1 MiB limit on a single NUL-terminated string inside a message. Under `fail`, the driver closes the connection when a message is over one of those limits; under `disable`, it skips them and reads the message anyway. No other check changes: a `maxCopyDataSize` you set, the `RowDescription` and `NegotiateProtocolVersion` limits, the authentication-message limits, and every limit that applies before the server has authenticated stay in force in both modes. An unset, empty or unrecognized value selects `fail`, and an unrecognized one is logged at `WARNING`: `System property pgjdbc.protocolHardeningMode has the unrecognized value <value>; expected fail or disable. Using fail, so the protocol limits stay enforced.` [Protocol message limits](#protocol-message-limits) lists every limit with the mode it is skipped in.
...
Either style is limited to 90% of max heap memory, and a larger value is lowered to that. The size must be positive: a value of zero or below is rejected when the connection opens, with `The maxResultBuffer connection property must be a positive size, but its value is <value>. Give a byte count such as 150M or a share of the heap such as 10p, or leave the property unset.` A value that is not in either style, such as `10%` or `abcM`, is rejected when the connection opens with `The maxResultBuffer connection property has the value <value>, which is not a valid size. Give a byte count such as 150M or a share of the heap with the p suffix, such as 10p.` Both errors carry SQLState `22023`.\
Rows are added up as they are read, and the count restarts at each `ReadyForQuery`, so an earlier round trip never counts against a later one. A round trip usually covers a whole `execute` call, but the driver can split a batch, or a call that runs several statements, into several round trips. When the driver reads a result through a cursor, each fetch batch is a round trip of its own, so the limit applies to one batch rather than to the whole result set. The driver uses a cursor only where all of the following hold:
  * the result set is forward-only and non-holdable;
  * a fetch size is set, with `setFetchSize` or `defaultRowFetchSize`;
  * auto-commit is off, or a transaction is open;
  * the query goes out on the extended protocol, which excludes every query under `preferQueryMode=simple`, and every statement other than a `PreparedStatement` under `preferQueryMode=extendedForPrepared`.

  Otherwise the whole result is read in one round trip.\
A row that does not fit is rejected before any of its body is read, and its body is then skipped, so the connection stays usable. Only the first skipped row in a round trip raises an error, so a result read through a cursor can report once per fetch batch. A row whose unread body exceeds 64 MB (64000000 bytes) cannot be skipped, and the driver closes the connection instead. Every error below carries SQLState `08S01`:

  | Row | Result | Error |
  | --- | --- | --- |
  | Over the limit on its own | The row is skipped, the call that read it fails, and the connection stays usable | `Result set exceeded maxResultBuffer limit. A single row of <row size> bytes exceeds the limit of <limit> bytes, so the row was skipped.` |
  | Fits on its own, but not with the rows already buffered | The row is skipped, the call that read it fails, and the connection stays usable | `Result set exceeded maxResultBuffer limit. The rows buffered since the last ReadyForQuery hold <buffered> bytes, so a row of <row size> bytes does not fit within the limit of <limit> bytes and was skipped.` |
  | Either of the above, with an unread body over 64 MB (64000000 bytes) | The connection is closed | `Result set exceeded maxResultBuffer limit. A row of <row size> bytes does not fit within the limit of <limit> bytes, and its <unread> unread bytes cannot be skipped, so the connection is closed.` |

  Where the connection stays usable, an application can catch the error and retry with a query that selects less. `COPY` output and replication data do not count against this property; `maxCopyDataSize` bounds those. `maxResultBuffer` limits memory rather than the length of a message, so `pgjdbc.protocolHardeningMode` does not skip it.
  Since: 42.7.14 for row skipping, for the three errors above, and for the rejection of `-1` and of values such as `10%`.
...
The driver fails the `COPY` on a message over the limit and closes the connection. With the property set, the error is `CopyData message has length <length>, which exceeds the maxCopyDataSize limit of <limit> bytes.`; while it is unset, the error names the built-in limit and adds the remedies: `Protocol error. CopyData message has length <length>, which exceeds the built-in limit of <limit> bytes. Raise the maxCopyDataSize connection property if the backend legitimately sends more, or set -Dpgjdbc.protocolHardeningMode=disable to skip these limits altogether.` Both carry SQLState `08S01`.\
Raise the property when a `COPY` or a replication slot legitimately sends larger messages. A value you set applies in both [protocol hardening modes](#protocol-message-limits), since you chose the number; `pgjdbc.protocolHardeningMode=disable` skips only the built-in limit.
  Since: 42.7.14

* **`maxServerTextMessageSize (`*String*`)`** *Default `null`, which applies a built-in limit of 64 MB (64000000 bytes)*\
Largest `ErrorResponse`, `NoticeResponse`, `CommandComplete`, `ParameterStatus` or `NotificationResponse` the driver accepts after the server has authenticated. The protocol fixes no maximum for the first four, so a `RAISE NOTICE` message or an error `DETAIL` arrives at whatever size the server produced; a `NotificationResponse` is bounded already, because PostgreSQL caps a `NOTIFY` payload at 8000 bytes.\
...
The driver closes the connection on a message over the limit, with `Protocol error. <message type> message has length <length>, which exceeds the pgjdbc limit of <limit> bytes. Raise the maxServerTextMessageSize connection property if the backend legitimately sends more, or set -Dpgjdbc.protocolHardeningMode=disable to skip these limits altogether.` The caller gets SQLState `08006` during a query and `08P01` during connection setup, as [Protocol message limits](#protocol-message-limits) describes.\
Raise the property if the server emits notices or error details larger than 64 MB (64000000 bytes). `pgjdbc.protocolHardeningMode=disable` skips this limit, whether or not the property is set. An `ErrorResponse` that arrives before the server has authenticated has a fixed limit of 30000 bytes instead, which neither the property nor [`pgjdbc.protocolHardeningMode`](#protocol-message-limits) changes.
  Since: 42.7.14
...
The driver checks the declared length of a backend message before it allocates anything to hold the message. A message over the limit that applies fails with `Protocol error. <message type> message has length <length>, which exceeds the pgjdbc limit of <limit> bytes.` Where `disable` skips the limit, the error continues with the remedy: `Raise the <property> connection property if the backend legitimately sends more, or set -Dpgjdbc.protocolHardeningMode=disable to skip these limits altogether.` The `CopyData` errors differ and are quoted in the `maxCopyDataSize` entry under [Connection Parameters](#connection-parameters); a message rejected before the server has authenticated carries the different text quoted under [Limits no setting relaxes](#limits-no-setting-relaxes). The driver then closes the connection and marks it broken, so `Connection.isClosed()` returns `true` and a pool that tests a connection on borrow discards it rather than reusing a stream whose position is unknown.

Most refusals reach the caller differently in the two phases. During a query one arrives as `An I/O error occurred while sending to the backend.` with SQLState `08006`, and the refusal quoted above is its cause. During SSL or GSS negotiation, authentication, and the startup messages that follow it, a length or framing check arrives as SQLState `08P01` with the message `The connection attempt failed: Protocol error. …`, while the two handshake-iteration limits report their own message with the same SQLState. A `CopyData` refusal reaches the caller as it is thrown, with SQLState `08S01`. A failure of the network connection itself, such as a reset or a socket timeout, reports `08001`.

A limit whose right value depends on the workload is a connection property, and raising it leaves the other limits in force. The driver sets every other limit itself. `pgjdbc.protocolHardeningMode=disable` skips only the limits marked `yes`.

| Message | Limit | Property that raises it | `disable` skips it |
| --- | --- | --- | --- |
| `CopyData`, which carries `COPY ... TO STDOUT` output and replication data | 64 MB (64000000 bytes) | `maxCopyDataSize` | yes, while the property is unset |
...
| One NUL-terminated string inside a message | 1 MiB (1048576 bytes) | none | yes, inside a tracked message |
...
A tracked message is one whose declared length the driver has already read, and that length bounds the scan in place of the string limit, which is why `disable` skips the limit there. A string read outside such a message keeps the 1 MiB limit in both modes. Before the server has authenticated the enclosing message is itself limited to 1 MiB or less, so the mode changes nothing there. Inside a message the failure is `Protocol error. C-string in <message type> message of <length> bytes exceeds the pgjdbc limit of <limit> bytes on a single C-string. Set -Dpgjdbc.protocolHardeningMode=disable to skip these limits altogether.`, and outside one it is `Protocol error. A C-string read outside a tracked message exceeds the pgjdbc limit of <limit> bytes on a single C-string.`

The `RowDescription` limit sits far above what PostgreSQL sends: a result of 1664 columns, the most PostgreSQL returns, needs about 133 KiB against the 8 MiB limit.

### Limits no setting relaxes

Each check below closes the connection in both modes. It rejects a length no wire-compatible backend sends, and reading on would leave the driver blocked on the socket indefinitely, or give a later query a result read out of the middle of an abandoned message.

* A declared length outside the range the protocol allows for that message type. No such range extends past 1073741823 bytes (just under 1 GiB), the largest allocation the backend itself makes: `Protocol error. <message type> message has invalid length <length> (expected between <min> and <max>).` A GSS encryption handshake token is bounded the same way, at 65532 bytes.
* A message whose reader consumed fewer bytes than its declared length, `Protocol error. <message type> message has <count> unread bytes.`, or more, `Protocol error. <message type> message was read <count> bytes past its declared length.`
* A stream that is not on a message boundary when the next message begins: `Protocol error. Reading the <message type> message stopped with <count> bytes of its body unread, so the connection is no longer positioned on a message boundary.` The read of the previous message stopped partway, usually because the network failed mid-message, and the check stops the next query on that connection from reading its first message header out of the abandoned body.
* A field count whose per-field entries do not fit the message, such as `Protocol error. DataRow field count <count> requires at least <required> bytes for per-field length prefixes, but the message size is only <size>.` or, for a `CopyInResponse`, `CopyOutResponse` or `CopyBothResponse`, `Protocol error. <message type> field count <count> requires message size <required>, but the message is <size> bytes.`
* A field or value that claims more bytes than its message still holds, which is the failure behind [issue #4015](https://github.com/pgjdbc/pgjdbc/issues/4015): `Protocol error. DataRow field <index> length <length> exceeds remaining row bytes <remaining>.` or `Protocol error. FunctionCallResponse value length <length> exceeds the <remaining> bytes left in the message.`
* Every limit that applies before the server has authenticated, since nothing has yet established who the peer is: `Protocol error. <message type> message has length <length>, which exceeds the pgjdbc limit of <limit> bytes applied before authentication. This limit cannot be relaxed.`
* A GSS encrypted packet whose declared length is outside 1 to 16380 bytes, the range libpq and the backend both enforce: `Protocol error. GSS encrypted packet has invalid length <length> (expected between 1 and <max> bytes).`
* An authentication exchange still unfinished after 64 authentication messages, `Protocol error. Authentication did not complete within <count> authentication messages.`, and a GSS encryption or GSS authentication handshake still unfinished after 64 round-trips, `Protocol error. GSS encryption handshake did not complete within <count> round-trips.` and `Protocol error. GSS authentication did not complete within <count> round-trips.` A zero-length GSS token is a valid continuation, so without these limits a server that answers every token with another one keeps the client looping before any credentials have been exchanged.

`maxResultBuffer` limits the memory a result set may occupy rather than the length of a message, so no mode skips it either; see its entry under [Connection Parameters](#connection-parameters).

Two further errors report a defect in the driver rather than anything the server sent. [Open an issue](https://github.com/pgjdbc/pgjdbc/issues) if you see `Protocol error. The <message type> message was read without a closing endMessage call, which is a pgjdbc defect.` or `Protocol error. The stream is <count> bytes away from the end of the <message type> message, so the next byte is not a message type. Read every backend message through readMessageLength or readFixedMessageLength, and close it with endMessage.`

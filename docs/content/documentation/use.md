...
The driver fails the `COPY` on a message over the limit and closes the connection, with `Protocol error. CopyData message has length <length>, which exceeds the maxCopyDataSize limit of <limit> bytes.` While the property is unset, the error names the built-in limit and adds the remedies: `Protocol error. CopyData message has length <length>, which exceeds the built-in limit of <limit> bytes. Raise the maxCopyDataSize connection property if the backend legitimately sends more, or set -Dpgjdbc.protocolHardeningMode=disable to skip these limits altogether.` Both carry SQLState `08S01`.\

...
### Fixed
* fix: a JDBC escape whose keyword is followed by a tab or a newline instead of a plain space is now processed. `{d\t'1999-01-09'}` was passed through untouched and the server answered with a syntax error on the brace. The separator may now be any character the backend scanner treats as whitespace, for `{d}`, `{t}`, `{ts}`, `{fn}`, `{oj}` and `{escape}` [PR #4392](https://github.com/pgjdbc/pgjdbc/pull/4392)

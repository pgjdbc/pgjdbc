...
### Added
* feat: add `Fastpath.fastpath(int|String, FastpathArg[], byte[], int, int)` and the matching `QueryExecutor.fastpathCall` overload, which store the result of a fastpath call in an array the caller supplies instead of allocating one per call. A result the caller has no room for throws `QueryExecutor.FastpathResultTooLongException`, SQLState `22023`. The response is read from the connection before that is thrown, so the connection stays usable [Issue #3043](https://github.com/pgjdbc/pgjdbc/issues/3043) [PR #4341](https://github.com/pgjdbc/pgjdbc/pull/4341)

### Changed
* **Breaking:** chore: `LargeObject.read(byte[], int, int)` now throws `ArrayIndexOutOfBoundsException` for a negative offset or length, or for a range that runs past the end of the array, and throws it before it sends the request. That is what `InputStream.read(byte[], int, int)` requires. A negative length used to return 0, because the server clamps the request to zero bytes. A negative offset, or one past the end of the array, used to return 0 whenever the object had no data left and to throw otherwise, and a length past the end of the array worked whenever the object had few enough bytes left to fit after the offset, so the same arguments succeeded or failed depending on the read position. Pass `buf.length - off` as the length to read whatever the object has left. A call whose range lies inside the array behaves as before [Issue #3043](https://github.com/pgjdbc/pgjdbc/issues/3043) [PR #4341](https://github.com/pgjdbc/pgjdbc/pull/4341)
* perf: `LargeObject.read(byte[], int, int)` now receives the reply straight into the caller's array instead of allocating a chunk-sized array per round trip and copying it. Reading a 1 MiB object in 64 KiB chunks over a plaintext connection allocates 7629 bytes in total, where it used to allocate 1055902. A `Blob` stream takes the direct path when the outstanding request is at least as large as the buffer it would otherwise refill; a smaller read still goes through the allocating call [Issue #3043](https://github.com/pgjdbc/pgjdbc/issues/3043) [PR #4341](https://github.com/pgjdbc/pgjdbc/pull/4341)

### Fixed
* fix: a `FunctionCallResponse` that declares a value length below `-1` now closes the connection and is reported as `PSQLState.PROTOCOL_VIOLATION` (SQLState `08P01`). Such a length reached `new byte[valueLen]` and threw `NegativeArraySizeException`, which is not an `IOException`, so the driver did not abort and handed the connection back in the middle of a message [PR #4341](https://github.com/pgjdbc/pgjdbc/pull/4341)

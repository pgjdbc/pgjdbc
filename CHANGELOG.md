...
### Fixed
* fix: `DatabaseMetaData.getBestRowIdentifier` falls back to a unique index when the table has no primary key, instead of returning an empty result. Exactly one index is reported: the primary key if there is one, then an index whose columns are all `NOT NULL`, then the narrowest, and finally the oldest, so the answer is stable. Invalid, partial and expression indexes are skipped, and an index with a nullable column is reported only when the caller passes `nullable = true` [PR #3656](https://github.com/pgjdbc/pgjdbc/pull/3656)

...
* fix: a slow answer to `CopyDone` no longer costs the connection. `endCopy` reports the read timeout and keeps the copy, the way `readFromCopy` already does for the read before it: CopyDone is on the wire before the wait starts, so waiting again re-sends nothing. Cancelling a copy that has already been told to finish now drains its answer instead of sending CopyFail to a backend that has left copy mode, and `PGCopyOutputStream.close()` gives the copy up rather than leaving it holding the connection

...
* fix: a null column-index array no longer fails `Connection.prepareStatement(String, int[])` and `Statement.execute(String, int[])` with SQLState `0A000`. A null array names no column and requests no generated keys, as `executeUpdate(String, int[])` and `executeLargeUpdate(String, int[])` already treated it. An array that names an index is still refused [PR #4427](https://github.com/pgjdbc/pgjdbc/pull/4427)

...
### Fixed
* fix: discarding the rest of a backend message no longer throws `java.net.SocketTimeoutException: Read timed out` on a connection that has run a replication stream with `socketTimeout=0`, the default, when the rest arrives later than the status interval (10 seconds unless set with `withStatusInterval`). The driver uses that interval as the socket timeout, and a read waits it out; a discard now waits it out too. The driver discards through its own buffer and no longer calls `skip` on the socket's input stream, including a stream supplied through the `socketFactory` property [PR #4370](https://github.com/pgjdbc/pgjdbc/pull/4370)
* fix: `PGStream.skip` no longer loops forever when end of stream is reached while the driver is discarding bytes, and throws `EOFException` instead. It also no longer loops forever on a stream supplied through the `socketFactory` property whose `skip` returns zero while data is still coming [PR #4358](https://github.com/pgjdbc/pgjdbc/pull/4358)

...
### Fixed
* fix: `BlobInputStream.read(byte[], int, int)` now throws `IndexOutOfBoundsException` for a range that does not fit the array, as `InputStream` requires. A negative length used to come back as `-1`, which a caller reads as end of stream, so a length computed to a negative value looked like a truncated large object; the other bad ranges failed inside `System.arraycopy` only after the driver had read from the server, leaving the large object positioned past bytes the caller never received. This reaches applications through the streams `LargeObject.getInputStream()` and `Blob.getBinaryStream()` return [PR #4385](https://github.com/pgjdbc/pgjdbc/pull/4385)

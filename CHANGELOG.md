...
* fix: a replication stream no longer takes the connection's socket timeout for its own. The stream has to stop waiting once per status interval so that a blocking `read()` can send a standby status update, and it got that by shortening the connection's socket timeout for as long as it ran. `start()` then failed with "Database connection failed when starting copy" whenever the server needed longer than one status interval to answer `START_REPLICATION`, and the shortened timeout was left behind after `close()`, so the next `START_REPLICATION` on that connection failed the same way. The wait is now the stream's own, and the connection keeps the timeout it was opened with [PR #4354](https://github.com/pgjdbc/pgjdbc/pull/4354)
* fix: `PGReplicationStream.read()` now sends the periodic standby status update on a connection opened without `socketTimeout`. The wait that ends a blocking read was put on the socket without recording that the driver wanted read timeouts reported, so it was swallowed and retried: `withStatusInterval` was ignored on the blocking path, and the server learned the flushed LSN only when it asked for a status update itself [PR #4354](https://github.com/pgjdbc/pgjdbc/pull/4354)
* fix: a read timeout partway through a backend message no longer desynchronizes a COPY stream. The bytes already taken are unrecoverable, so the next read took the middle of that message for the start of the next one and reported `Unexpected packet type during copy`. A replication stream's wait now covers only the byte that starts a message, and a message that stops arriving part-way ends the connection instead of looking like something to resume from [PR #4354](https://github.com/pgjdbc/pgjdbc/pull/4354)

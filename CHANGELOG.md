...
### Fixed
* fix: the parser no longer drops the `;` between statements when it is not splitting them. `Parser.parseJdbcSql` with `splitStatements=false` glued the statements together, so `insert into t values(1); insert into t values(2)` came back as one statement running into the next and the server answered `42601 syntax error`. That combination is reached by `Statement.executeUpdate(sql, columnNames)` under `preferQueryMode=simple` and `extendedForPrepared`, which now runs both statements and reports the first statement's update count. A `RETURNING` clause is no longer added to such a query at all, and `getGeneratedKeys()` returns an empty result for it, because the driver reads the first result the server sends as the generated keys and with more than one statement that result belongs to another statement [PR #4378](https://github.com/pgjdbc/pgjdbc/pull/4378)

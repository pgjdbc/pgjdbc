...
### Fixed
* fix: `BlobInputStream.mark()` no longer logs a SEVERE record with a stack trace when the stream is closed. Marking a closed stream has no effect, which the method documents, and an application that closes a buffered stream marking as it goes saw that record for every mark [PR #4389](https://github.com/pgjdbc/pgjdbc/pull/4389)

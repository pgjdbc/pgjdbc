...
### Fixed
* fix: `PreparedStatement.setObject` now binds a `PGbox[]` as `box[]`. It used to fail with `Cannot cast an instance of [Lorg.postgresql.geometric.PGbox; to type Types.ARRAY`, because the driver's built-in type table spelled the element class `org.postgresql.geometric.PGBox` [PR #4419](https://github.com/pgjdbc/pgjdbc/pull/4419)

...
### Fixed
* fix: `VisibleBufferedInputStream.ensureBytes` now refuses a negative byte count with an `IOException` instead of accepting it. A backend message whose declared length is shorter than its own header made `PGStream.receiveString` decode with a negative length and throw `StringIndexOutOfBoundsException`, which escapes the driver's `IOException` recovery. A count near `Integer.MIN_VALUE` overflowed the buffer-shortfall computation instead and grew the buffer until the JVM ran out of heap. The class is public, so a caller outside the driver that passed a negative count and relied on the `true` now gets an `IOException` [PR #4375](https://github.com/pgjdbc/pgjdbc/pull/4375)

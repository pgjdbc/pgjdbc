...
### Fixed
* fix: a replication stream's wake-up no longer interrupts a backend message it has started reading. The stream stops waiting once per status interval so a blocking read can send a standby status update, and it did so wherever the read happened to be. A wait that ran out partway through a message dropped the bytes already taken, and the next read took the middle of that message for the start of the next one, reporting `Unexpected packet type during copy`. The wait now covers only the byte that starts a message, and the connection's own `socketTimeout` governs everything else
...
* fix: a replication stream now puts the connection's socket timeout back when it ends. The stream shortens the timeout to the status interval so that a blocking read wakes up to send a standby status update, but it never restored it, so every later operation on that connection — including the `START_REPLICATION` of a second stream — ran with a socket timeout of a few milliseconds [PR #4354](https://github.com/pgjdbc/pgjdbc/pull/4354)
* fix: `PGReplicationStream.read()` now sends the periodic standby status update on a connection opened without `socketTimeout`. The stream shortens the socket timeout to the status interval and uses the resulting read timeout as its wake-up, but a connection that did not ask for read timeouts had them swallowed and retried, so `withStatusInterval` was ignored on the blocking read and the server learned the flushed LSN only when it asked for a status update itself [PR #4354](https://github.com/pgjdbc/pgjdbc/pull/4354)

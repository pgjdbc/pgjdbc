...
### Fixed
* fix: `GSSInputStream` now reports the decrypted bytes of the current GSS frame that it has not yet handed out. A frame carries more plaintext than a single read takes, and `available()` returned 0 for the remainder, so `PGStream.hasMessagePending` missed it and fell back to a probe rate-limited to one attempt per second. Under GSS encryption that delayed asynchronous notifications and cut non-blocking `COPY` processing short [PR #4383](https://github.com/pgjdbc/pgjdbc/pull/4383)

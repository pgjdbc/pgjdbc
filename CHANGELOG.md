...
### Fixed
* fix: parse `IntervalStyle=sql_standard` intervals transferred as text (`binaryTransfer=false` or `binaryTransferDisable=interval`) without silently zeroing the year, month, and day fields. Malformed SQL-standard-like literals and values outside the integer field range of `PGInterval` now throw `SQLException` instead of returning zeros. [Issue #4294](https://github.com/pgjdbc/pgjdbc/issues/4294) [PR #4296](https://github.com/pgjdbc/pgjdbc/pull/4296)

...
### Fixed
* fix: `Blob.getBinaryStream(pos, length)` now rejects a position below 1 and a negative length with `INVALID_PARAMETER_VALUE` instead of passing them through. Position 0 became a seek to `-1`, which the server refuses and which aborts the caller's transaction; a length of `-1` collided with the driver's internal "no limit" sentinel and returned the whole large object, so callers that used it to mean "the rest of the blob" now get an exception [PR #4382](https://github.com/pgjdbc/pgjdbc/pull/4382)
* fix: a `BlobInputStream` limit near `Long.MAX_VALUE` no longer serves an empty stream. The limit is stored relative to the large object's position and made absolute on first use, and that addition overflowed to a value behind the position, so `LargeObject.getInputStream(Long.MAX_VALUE)` returned `-1` for every read unless the object sat at offset 0. The absolute limit now saturates [PR #4382](https://github.com/pgjdbc/pgjdbc/pull/4382)

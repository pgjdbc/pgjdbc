...
* feat: add the `maxServerTextMessageSize` connection property, the largest `ErrorResponse`, `NoticeResponse`, `CommandComplete`, `ParameterStatus` or `NotificationResponse` the driver accepts, where the protocol fixes no maximum of its own. It is unset by default, which applies a built-in limit of 64 MB (64000000 bytes), large enough for any workload that works today. `RowDescription`, `NegotiateProtocolVersion`, the pre-authentication token messages, and an `ErrorResponse` that arrives before authentication (30000 bytes, as in libpq) have fixed limits instead, since none of them varies with the workload [Issue #4015](https://github.com/pgjdbc/pgjdbc/issues/4015) [PR #4016](https://github.com/pgjdbc/pgjdbc/pull/4016)
...
### Changed
* fix: a length or framing check that rejects a backend message during SSL or GSS negotiation, authentication, or the startup messages that follow it now reports SQLState `08P01` (`PSQLState.PROTOCOL_VIOLATION`) with the message `The connection attempt failed: Protocol error. …` and the rejection as the cause, like the other protocol violations the driver reports during connection setup. A failure of the network connection itself, such as a refused connection, a reset, end of stream, a socket timeout, or an SSL failure, still reports `08001`. Code that checks the SQLState class `08`, such as HikariCP's `startsWith("08")`, Spring's `SQLStateSQLExceptionTranslator`, and `PGPooledConnection`, still classifies the failure as a connection error; code that compares the SQLState with `"08001"` exactly, or calls `PSQLState.isConnectionError`, no longer does [Issue #4015](https://github.com/pgjdbc/pgjdbc/issues/4015) [PR #4016](https://github.com/pgjdbc/pgjdbc/pull/4016)

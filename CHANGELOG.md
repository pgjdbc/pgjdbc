...
### Fixed
* fix: the large object failure messages from `BlobOutputStream` now print the OID as an identifier rather than as a quantity. `GT.tr` formats through `MessageFormat`, which groups a number by the reader's locale, so `Can not flush large object 2180387` arrived as `2,180,387` and could not be pasted into `lo_unlink` or a `pg_largeobject` query. The message texts are unchanged, so translations are unaffected [PR #4372](https://github.com/pgjdbc/pgjdbc/pull/4372)

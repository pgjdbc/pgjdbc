...
### Fixed
* fix: `FixedLengthOutputStream`, which carries a `ByteStreamWriter`'s bytes to the server, no longer accepts a range whose `offset + len` overflows `int`. Only an application's own `ByteStreamWriter` can hand it one, since neither implementation the driver ships derives a range that overflows. The target it forwards to does not always bound its own copy - the one that builds a `bytea` literal for a simple query bounds its loop with the same overflowing addition - so such a range could be counted as written without any of its bytes being copied, leaving the value short by that many zero bytes and reporting nothing. Where the overflow came from the length rather than the offset, the range was reported as exceeding the declared size instead of as the bad range it is [PR #4395](https://github.com/pgjdbc/pgjdbc/pull/4395)

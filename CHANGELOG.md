...
### Fixed
* fix: asking for generated keys on multi-statement SQL no longer fails. `PreparedStatement.executeUpdate()` threw `A result was returned when none was expected` (SQLState `0100E`) in every query mode, because the driver appended a `RETURNING` clause to every `INSERT`, `UPDATE`, `DELETE` and `WITH` statement it found, while the JDBC layer reported no generated keys for the query at all. A `PreparedStatement` splits the statements in every mode, but under `preferQueryMode=simple` and `extendedForPrepared` a query that carries no parameters does not, and there the parser also dropped the semicolon that separated them, so `Statement.executeUpdate(String, String[])` got `42601 syntax error` instead; the dropped semicolon reached statements that never got a `RETURNING` clause too, such as `select 1; select 2`. The semicolon now survives, and the column names are ignored for multi-statement SQL, which JDBC allows when a statement cannot return generated keys. Every mode now reports the update count of one statement and an empty `getGeneratedKeys()`, where the default `extended` mode previously returned `-1` from `Statement.executeUpdate(String, String[])` and `true` from `Statement.execute(String, int)`. A statement followed by a trailing comment, such as `insert ...; -- done`, now reports its update count instead of `-1`; it reported no generated keys before this change and still reports none. One case changes on its own account: a comment terminated by a semicolon counts as a statement, so `/*c*/; insert ...` is multi-statement below extended mode and no longer reports generated keys. Extended mode splits that SQL into two statements and has always answered the same way [PR #4365](https://github.com/pgjdbc/pgjdbc/pull/4365)

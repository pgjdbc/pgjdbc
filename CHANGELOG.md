...
### Fixed
* fix: `BlobOutputStream` now ends its writes on the large object's own row boundaries rather than on a count of the bytes it happens to have written. PostgreSQL stores a large object in `LOBLKSIZE` rows and rereads any row a write ends inside, which the stream avoids by holding a remainder back, but it counted from zero: a stream obtained from `Blob.setBinaryStream(long)` at a non-zero position, or any stream after an explicit `flush()`, ended every later write mid-row and gave up the very thing the remainder was held back for. The stream asks the server where it is once, and once more after each `flush()`, and moves the offset itself the rest of the time; a stream whose buffer is too small to hold a remainder never asks at all. Single-byte writes hold a remainder back as well, so they establish the alignment instead of only preserving one the stream already had. Seeking the large object under an open stream now needs a `flush()` first, which it always did in practice, since without one the buffered bytes land at the new position rather than the one they were written for [PR #4371](https://github.com/pgjdbc/pgjdbc/pull/4371)

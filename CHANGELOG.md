...
### Fixed
* fix: `VisibleBufferedInputStream.skip` now returns `0` and leaves the read position unchanged for a count of zero or less. It used to add the count to the read position and report it as skipped, so a negative count could drive the position below the start of the buffer, after which `available()` over-reported and the next read threw `ArrayIndexOutOfBoundsException` instead of an `IOException` the driver turns into a connection failure. Only code that calls `VisibleBufferedInputStream` directly is affected, since inside the driver the decode ahead of every `skip` throws first [PR #4359](https://github.com/pgjdbc/pgjdbc/pull/4359)

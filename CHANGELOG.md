...
### Fixed
* fix: `/*/` is now an unterminated block comment for the driver, as it is for the server. The `*` that opens a comment used to be allowed to close it, so the driver parsed the text after `/*/` as SQL code: a `?` became a bind parameter, a `;` split the statement, and a `'` opened a string literal, which made the driver reject `SELECT /*/ ' */ 1` even though PostgreSQL accepts it. Escape processing now raises `PSQLException` for SQL whose block comment is never closed [PR #4363](https://github.com/pgjdbc/pgjdbc/pull/4363)

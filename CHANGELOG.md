...
### Fixed
* fix: `Parser.parseInsertKeyword`, `parseBeginKeyword` and `parseAtomicKeyword` no longer demand one character more than they read, so a keyword that ends at the end of the input is recognised. The other ten matchers in the class always asked for exactly what they compare [PR #4394](https://github.com/pgjdbc/pgjdbc/pull/4394)

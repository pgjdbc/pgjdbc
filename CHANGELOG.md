...
### Fixed
* fix: `Parser.parseSingleQuotes` now reads an `E''` literal whose `E` is the first character of the input the same way as one anywhere else. The escape-string check required a character before the `E`, so `parseSingleQuotes("E'\\''".toCharArray(), 1, true)` returned 3, taking the escaped quote for the closing one, while `parseSingleQuotes(" E'\\''".toCharArray(), 2, true)` returned 5, the closing quote. The driver passes the method the whole statement text, and no statement PostgreSQL accepts begins with a string literal, so the driver parses every valid statement as before. The method is public, and a caller that passes an offset of its own sees the difference [PR #4396](https://github.com/pgjdbc/pgjdbc/pull/4396)

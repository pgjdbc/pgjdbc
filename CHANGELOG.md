...
### Fixed
* fix: a result set read through a cursor no longer returns every remaining row, ignoring `Statement.setMaxRows`, when the fetch size is close to `Integer.MAX_VALUE`, for example after `ResultSet.setFetchSize(Integer.MAX_VALUE)` or with an `adaptiveFetch` size that large. The rows already read plus the fetch size overflowed to a negative `int`, so `next()` and `isLast()` skipped the `maxRows` limit on the next fetch [PR #4431](https://github.com/pgjdbc/pgjdbc/pull/4431)
* fix: `ResultSet.isLast()` no longer lets a result set read through a cursor (`setFetchSize` with auto-commit off) return one row more than `Statement.setMaxRows` allows. On the last row `maxRows` allows, `isLast()` returned `false` and fetched the next row; on an earlier row at the end of a fetched block, it fetched one row too many. `next()` then returned the extra row [PR #4431](https://github.com/pgjdbc/pgjdbc/pull/4431)

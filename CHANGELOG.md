...
### Fixed
* perf: on a statement cache miss, `prepareStatement` and `prepareCall` parse the SQL without holding the connection's statement cache lock, so another thread that uses the same connection does not wait for the parse [Issue #3835](https://github.com/pgjdbc/pgjdbc/issues/3835) [PR #3836](https://github.com/pgjdbc/pgjdbc/pull/3836)

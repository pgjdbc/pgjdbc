...
### Fixed
* fix: `BlobInputStream` now sizes its read buffer for the stream limit, so `Blob.getBinaryStream(pos, length)` no longer asks the server for a full 64 KiB buffer to answer a read on a stream that may serve ten bytes [PR #4386](https://github.com/pgjdbc/pgjdbc/pull/4386)
* fix: the `BlobInputStream` buffer growth policy no longer overflows. A read request above 2^30 produced a negative buffer size, and a caller-supplied buffer ceiling above 2^30 made the buffer collapse from 1 GiB back to 2 bytes and grow again. A buffer size that is not positive is now refused instead of producing a stream that reports end of stream on its first read [PR #4386](https://github.com/pgjdbc/pgjdbc/pull/4386)

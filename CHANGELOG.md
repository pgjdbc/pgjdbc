...
### Fixed
* fix: reading a closed connection's stream now raises an `IOException` instead of answering out of the buffer it held when the connection went away. Those bytes read as ordinary protocol data until the buffer runs out, so the failure surfaced somewhere unrelated to the close. `PGStream.close` and `QueryExecutorCloseAction.close` also close everything whatever an earlier step throws, and report the first failure with the later ones suppressed; before, a connection that had already dropped failed its flush and left both the socket and the input stream open [PR #4390](https://github.com/pgjdbc/pgjdbc/pull/4390)

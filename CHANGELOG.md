...
### Fixed
* fix: `BlobOutputStream` no longer miscomputes its write plan for one `write(byte[], int, int)` whose length approaches `Integer.MAX_VALUE`. The total of buffered plus incoming bytes was computed in `int`, so it wrapped negative: the stream skipped the flush it owed, sized its buffer from the negative total and got an empty one, dropped the bytes it had already buffered, and threw `ArrayIndexOutOfBoundsException` instead of writing. The total is computed in `long` now, and a write larger than one `lowrite` can carry as a single `bytea` is split into several instead of being sent as one call the server refuses. The buffer size passed to the `BlobOutputStream(LargeObject, int)` constructor is capped at 512MiB for the same reason; it was already rounded down to a power of two [PR #4366](https://github.com/pgjdbc/pgjdbc/pull/4366)

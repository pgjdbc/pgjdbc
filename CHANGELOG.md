...
### Fixed
* fix: `BlobOutputStream.close()` now releases the large object descriptor and marks the stream closed even when the final flush fails. It flushed first and skipped `LargeObject.close()` when that threw, so the server-side descriptor stayed open until the transaction ended, a second `close()` retried the write that had already failed, and the stream went on accepting writes to a large object the caller believed was gone [PR #4364](https://github.com/pgjdbc/pgjdbc/pull/4364)

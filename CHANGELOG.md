...
### Fixed
* fix: a `bytea` parameter is no longer mangled on a server with `standard_conforming_strings=off`. Under `preferQueryMode=simple` the driver inlined the value as `'\xcafebabe'::bytea`, and such a server reads that leading backslash as an escape character: depending on the first byte, the statement failed with `ERROR: invalid byte sequence for encoding "UTF8": 0x00` or `ERROR: invalid input syntax for type bytea`, or the column silently took roughly twice the bytes that were sent. The driver now writes an escape string constant, `E'\\xcafebabe'::bytea` [PR #4404](https://github.com/pgjdbc/pgjdbc/pull/4404)

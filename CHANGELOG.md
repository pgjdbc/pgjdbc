...
### Fixed
* fix: with `adaptiveFetch=true`, a fetch no longer returns every remaining row at once when the computed fetch size is 0 or negative. `maxResultBuffer` divided by the largest row size was cast to `int`, so a buffer of 2147483648 bytes or more with small rows overflowed to a negative size, and a row larger than the buffer gave 0; the server reads a row limit of 0 or less as "all rows". The computed size is now at least 1 and at most `Integer.MAX_VALUE`, before `adaptiveFetchMinimum` and `adaptiveFetchMaximum` apply [PR #4430](https://github.com/pgjdbc/pgjdbc/pull/4430)

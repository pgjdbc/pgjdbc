...
### Fixed
* fix: a comment between a `{fn ...}` escape's name and its argument list no longer becomes part of the name. A line comment took the argument list with it, because the newline that ended it was trimmed away, so `{fn abs -- c<newline> (-1)}` reached the server as `abs -- c(-1)`. A block comment kept the driver from recognising a function it rewrites, so `{fn concat /* c */ ('a','b')}` was passed through instead of becoming `('a'||'b')`. The comment is removed from the SQL the driver sends, leaving the space that separated the tokens around it [PR #4397](https://github.com/pgjdbc/pgjdbc/pull/4397)
* fix: a `{fn name}` escape with no argument list is now reported as a `PSQLException` instead of being rewritten into nonsense. `select {fn now}` became `select ow`, because the first character of the name was dropped, and the search for the argument list ran past the closing brace, so `select {fn now} from (t)` picked up the parenthesis of `from (t)` and became `select now} from(t)`. A quoted function name whose text contains `(` is now handled too: `{fn "we(ird"()}` used to be rejected as an unterminated identifier. A statement that relied on the old mangling now fails at the driver with SQLSTATE 42601 instead of reaching the server [PR #4374](https://github.com/pgjdbc/pgjdbc/pull/4374)

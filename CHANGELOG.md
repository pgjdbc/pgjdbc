...
### Fixed
* fix: `BlobOutputStream.write(byte[], int, int)` now throws `IndexOutOfBoundsException` for a range that does not fit the array, as `OutputStream` requires. A range that ran past the end of the array was previously sent to the server, where `PGStream.send` pads it with zero bytes, so the large object silently gained zeros; the same call on a stream with a larger buffer threw `ArrayIndexOutOfBoundsException` instead. This reaches applications through the streams `LargeObject.getOutputStream()` and `Blob.setBinaryStream(long)` return. `Blob.setBytes(long, byte[], int, int)` writes the large object without going through `BlobOutputStream` and still pads an out-of-range request with zeros [PR #4362](https://github.com/pgjdbc/pgjdbc/pull/4362)

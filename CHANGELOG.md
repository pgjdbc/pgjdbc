...
* fix: a connection that sets `assumeMinServerVersion` from 9.0 to 11.x reaches a session that permits only specific queries again, such as one opened with `options=-c gp_retrieve_conn=true`. Since 42.7.6 the driver delivered `extra_float_digits` only through a post-authentication `SET extra_float_digits = 3`, which such a session rejects, so the connection failed during initialization. The driver now sends the value in the startup packet, as 42.7.3 did, and it costs no extra round trip. Behind PgBouncer such a connection is now refused with `unsupported startup parameter: extra_float_digits`: on PostgreSQL 12 or later set `assumeMinServerVersion=12`, and before 12 leave the property unset or add `extra_float_digits` to PgBouncer's `track_extra_parameters`. Adding it to `ignore_startup_parameters` alone makes a server older than 12 keep `extra_float_digits` at 0, so floats returned as text lose precision without an error; the [`assumeMinServerVersion` entry](https://jdbc.postgresql.org/documentation/use/#connection-parameters) lists every option. On a 12+ server, `extra_float_digits` now reads 3 rather than the server default of 1; float output is unchanged, because every value above 0 selects the same shortest form. A replication connection with `assumeMinServerVersion` from 9.4 to 11.x now runs with `extra_float_digits` at 3 on a 9.4 to 11.x server instead of the server default of 0, so floats that logical decoding renders as text for it can carry up to 3 more digits [Discussion #4306](https://github.com/pgjdbc/pgjdbc/discussions/4306) [PR #4308](https://github.com/pgjdbc/pgjdbc/pull/4308)

...
### Fixed
* fix: statements after a `CREATE FUNCTION ... BEGIN ATOMIC ... END` are split again. The parser stopped splitting on `;` as soon as it saw `BEGIN ATOMIC`, which is right inside the function body, but never noticed the `END` that closes it, so everything after the `CREATE` was glued to it and the server answered `cannot insert multiple commands into a prepared statement`. A `CASE` expression inside the body keeps its own `END`, and an `END` used as a column label does not end it: only an `END` that nothing but the statement separator follows counts. A body whose first statement is empty, written as `BEGIN ATOMIC;`, is no longer split at that semicolon either [PR #4379](https://github.com/pgjdbc/pgjdbc/pull/4379)

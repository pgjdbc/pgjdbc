...
* fix: a forced Sync now falls between batch entries rather than between the statements of one. The deadlock check ran per statement, so a batch large enough to fill the estimated receive buffer could Sync in the middle of a multi-statement entry, committing its leading statements on their own under auto-commit and letting a later failure report an entry that had already run. The check now covers a whole entry before any of it is sent, unless the entry's own responses reach the buffer limit — one that large cannot be sent unread, so it keeps the per-statement checks [Issue #4349](https://github.com/pgjdbc/pgjdbc/issues/4349) [PR #4350](https://github.com/pgjdbc/pgjdbc/pull/4350)

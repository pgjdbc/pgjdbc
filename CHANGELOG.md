...
### Fixed
* fix: the COPY write path now throws `IndexOutOfBoundsException` for a range that does not fit the array instead of sending the missing bytes as zeros. `PGStream.send` pads a range that runs past the end of the array rather than failing, so `PGCopyOutputStream.write(new byte[10], 5, 200000)` sent about 200KB of zeros into the COPY stream. Whether such a request was refused at all depended on how it compared to the stream's buffer, which the caller does not choose with that in mind. `CopyIn.writeToCopy(byte[], int, int)` is checked as well, since the implementation the copy API hands out does not go through `PGCopyOutputStream` [PR #4384](https://github.com/pgjdbc/pgjdbc/pull/4384)

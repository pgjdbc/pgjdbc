...
### Fixed
* fix: a `CallableStatement` whose body ends in a backslash inside a string constant now raises `PSQLException` instead of `StringIndexOutOfBoundsException`. With `standard_conforming_strings=off` the escape skipped the character after the backslash even when the input had ended, which stepped the scan past the end of the string and reached `substring()` before the malformed-escape check [PR #4369](https://github.com/pgjdbc/pgjdbc/pull/4369)
* fix: `CallableStatement` no longer ends the `{call ...}` escape at a brace that belongs to something else. A brace inside a delimited identifier or a dollar-quoted string ended it — `{call "we}ird"()}` and `{call f($$a}b$$)}` were rejected — and an apostrophe inside a block comment opened a string constant, so `{call f(?) /* don't */}` was rejected too. A comment before `call`, as in `{/*c*/? = call f(?)}`, is now accepted, the way one before the opening brace or after the closing one already was, and a line comment in the body is treated as text rather than SQL, so `{call f(?) -- don't}`, `{call f(?) -- ;}` and `{call f(?) -- returns {} on empty}` followed by a newline no longer fail. A call body holding an unterminated delimited identifier, dollar-quoted string or block comment is now rejected with SQLSTATE 42601 instead of being passed to the server, which rejected it in turn. The brace counter also fixes a nested JDBC escape in the body, which is reachable only by calling the parser directly, since escape processing expands those before the call parser runs [PR #4381](https://github.com/pgjdbc/pgjdbc/pull/4381)

...
### Fixed
* fix: `VisibleBufferedInputStream` no longer takes a wrapped stream that read nothing for one that has ended, nor counts it as progress. `GSSInputStream` reads nothing whenever a GSS frame is still incomplete, and the socket times out on timeouts the driver sets for itself such as the replication status interval. `read(byte[], int, int)` returned `0` for a non-empty request, which `InputStream` forbids, and `ensureBytes` looped over a request the buffer never satisfied. Both now block for a single byte, which reports the end of the stream as `-1`, and a timeout the caller did not ask for is waited out [PR #4360](https://github.com/pgjdbc/pgjdbc/pull/4360)

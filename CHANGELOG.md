...
### Fixed
* fix: `LargeObject.write(byte[], int, int)` now throws `IndexOutOfBoundsException` for a range that does not fit the array instead of storing the missing bytes as zeros. `PGStream.send` pads a range that runs past the end of the array rather than failing, so `Blob.setBytes(1, new byte[10], 5, 200)` stored 200 bytes: five from the caller and 195 zeros, with no exception. `Blob.setBytes` reaches this method directly, without passing through `BlobOutputStream`. A null array now throws `NullPointerException` rather than sending SQL NULL to `lowrite` [PR #4377](https://github.com/pgjdbc/pgjdbc/pull/4377)

...
### Fixed
* fix: a comment after the last `;` is no longer executed as a statement of its own. `SELECT 1; -- trailing` was split into two queries, so `Statement.execute` reported two results and `executeQuery` failed with SQLSTATE 0100E, "A query returned multiple ResultSets". Such a comment is now removed from the statement sent to the server; a comment inside a statement, or at the end of one that has no trailing `;`, is untouched. A query that is nothing but a comment is still a query [PR #4387](https://github.com/pgjdbc/pgjdbc/pull/4387)

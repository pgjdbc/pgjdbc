...
### Fixed
* fix: `PreparedStatement.addBatch()` on multi-statement SQL (`INSERT ...; DELETE ...`) now fails with a `PSQLException` that names the restriction instead of a `ClassCastException` from `CompositeQuery` to `SimpleQuery`. A batch reserves one update-count slot per entry while the server reports one `CommandComplete` per statement, so such an entry has never worked in any release; it is now refused as it is added, so no part of it is applied. `Statement.addBatch(String)` has refused the same input since 42.7.13 [Issue #4349](https://github.com/pgjdbc/pgjdbc/issues/4349)

...
### Fixed
* fix: an empty or `null` generated-key column name is now reported as a `SQLException`. `prepareStatement(sql, new String[]{""})` threw `StringIndexOutOfBoundsException` from the check for the `*` sentinel, and a `null` name threw `NullPointerException` a few lines later. That check is also an exact match now, so a column really named `*abc` is quoted as an identifier rather than turned into `RETURNING *` [PR #4368](https://github.com/pgjdbc/pgjdbc/pull/4368)

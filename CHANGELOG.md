...
### Fixed
* fix: `BEGIN ATOMIC` is recognised only where a function body can appear — inside a `CREATE FUNCTION` or `CREATE PROCEDURE`, as two adjacent keywords. Both words are ordinary identifiers in PostgreSQL, and the parser took any two consecutive keywords after any `CREATE`, so `create view v as select begin, atomic; insert into t(a) values(?)` and `create table begin(atomic int); select 1` glued the rest of the string to the `CREATE`. The routine is recognised by the keyword that names what the `CREATE` creates, so an alias or a column called `function` elsewhere in the statement does not count [PR #4367](https://github.com/pgjdbc/pgjdbc/pull/4367)
* fix: in multi-statement SQL, a statement that follows a `;` no longer picks up the wrong command type or loses the `RETURNING` clause the driver adds for generated keys. Three defects made the position matter: the keyword in front of a `;` was examined only after the split had reset the parser's flags, so `insert into dv default values; insert into t(a) values(?)` handed the first statement's flags to the second, which then got no `RETURNING`. `values` is not reserved in PostgreSQL, so a statement really can end on that keyword; only the first `INSERT` of the string was recognised as one; and a statement whose last character was the bind placeholder was mistaken for an empty trailing statement and inherited the command type of the statement before it, so `select 1; delete from t where a=?` was reported as a `SELECT` [PR #4367](https://github.com/pgjdbc/pgjdbc/pull/4367)

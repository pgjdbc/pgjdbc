...
### Fixed
* fix: `BlobInputStream.skip` no longer aborts the caller's transaction. The target was clamped to the stream limit, which is `Long.MAX_VALUE` unless the caller set one, and PostgreSQL refuses a large object seek beyond `INT_MAX * LOBLKSIZE`, so `skip(Long.MAX_VALUE)` always failed and left the connection unusable until rollback. A skip that would land past 512 GiB, the smallest such maximum any server build can have, now stops at the end of the large object, and `skip` returns the distance it actually covered. On a server older than 9.3, which has no `lo_lseek64`, that ceiling is 2 GiB [PR #4361](https://github.com/pgjdbc/pgjdbc/pull/4361)

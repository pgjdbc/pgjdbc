...
### Fixed
* fix: a parameter value with a backslash in it no longer produces a `nonstandard use of \\ in a string literal` warning on a server with `standard_conforming_strings=off`. Under `preferQueryMode=simple` the driver inlined it as a plain literal with the backslash doubled, and the warning came back as an `SQLWarning` on the statement, one per literal. It now goes into an escape string constant, `E'a\\b'`, which means the same under either setting; the hex `bytea` form is unchanged [PR #4405](https://github.com/pgjdbc/pgjdbc/pull/4405)

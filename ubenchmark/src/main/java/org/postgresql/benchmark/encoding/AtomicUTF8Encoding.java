...
/**
 * Similar to {@code org.postgresql.core.UTF8Encoding}, but manages reference to the member {@code char[]}
 * in an {@link AtomicReference} rather than making {@link #decode(byte[], int, int)} {@code synchronized}.
 */
final class AtomicUTF8Encoding extends Encoding {
  ...
  private final AtomicReference<char[]> decoderArrayRef = new AtomicReference<char[]>(new char[1024]);
  ...
  public String decode(byte[] data, int offset, int length) throws IOException {
    final char[] cdata = getArray(length);

 ...
 * {@code LiteralCursor}), and the scalar text decoders -- rather than only the well-formed literals the
 ...
 */
class RawTextLiteralDecodeFuzzTest {
  ...
  private static CodecContext contextWithPoint() {
    ...
    return PgCodecContext.offlineBuilder().type(point).build();
  }
